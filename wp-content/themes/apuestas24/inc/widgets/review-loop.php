...
vc_map( array(
	"name" => __("Reviews List V2"),
	"base" => "reviews-list-v2",
	"category" => __('Content'),
	"params" => array(
		array(
			"type" => "textfield",
...
add_shortcode('reviews-list-v2', 'goapostas_reviews_list_v2');
function goapostas_reviews_list_v2($atts,$content){
	 		...
	 		while ( $wp_query->have_posts() ) : $wp_query->the_post();
				...
				if ($counter <= 3) {
					$out .= '<div class="review-block-dark">';
						$mark = get_field( 'review_mark' )? get_field( 'review_mark' ) : '';
						$color_mark = get_field( 'color_review_mark' )? get_field( 'color_review_mark' ) : '';

						if ($mark && $mark != '' && $color_mark && $color_mark != '') {
							$out .= '<span class="review-mark" style="background-color:'.$color_mark.'">'.$mark.'</span>';							
						}
